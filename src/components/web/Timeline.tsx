      ...
      <div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row gap-12 lg:gap-24">
        ...
        <div className="lg:w-1/4">
          <div className="sticky top-32">
            <h3 className="text-6xl md:text-8xl font-bold tracking-tighter text-white">Main Day</h3>
          ...
          {schedule.map((item, index) => {
            ...
            if (isBreak) {
              return (
                <div
                  key={index}
                  className="flex flex-col md:flex-row items-center justify-center py-8 md:py-12 px-6 border-y border-white/10 bg-white/5 rounded-xl gap-4 my-4 group hover:bg-white/10 transition-colors duration-500 text-center"
                >
            ...
            return (
              ...
              >
                {/* Time */}
                <div
                  className={`md:w-48 flex-shrink-0 text-lg md:text-xl font-medium tracking-wide ${isGame ? "text-blue-300" : "text-gray-400 group-hover:text-[#3B82F6] transition-colors"}`}
                >
                  {item.time}
                </div>
                ...
                <div className="flex flex-col gap-3 w-full">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      {isGame ? (
                        ...
                        <svg
                          ...
                          viewBox="0 0 40 10"
                          fill="none"
                          className="w-5 h-2 mt-2 text-[#3B82F6]"
                        >
                          <path
                            d="M1 5C1 5 4.5 1 8.5 5C12.5 9 16 5 16 5C16 5 19.5 1 23.5 5C27.5 9 31 5 31 5C31 5 34.5 1 38.5 5"
                            stroke="currentColor"
                      ...
                      <h4
                        className={`text-2xl md:text-3xl lg:text-4xl font-bold leading-tight ${isGame ? "text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]" : "text-white"}`}
                      >
                        {item.title}
                      </h4>
                    </div>
                    ...
                    {/* Game Trophy Icon */}
                    {isGame && (
                      <Trophy className="w-8 h-8 text-yellow-400 drop-shadow-lg hidden sm:block" />
                    )}
                  </div>

                  {item.description && (
                    <p
                      className={`text-lg md:text-xl ml-8 ${isGame ? "text-gray-300" : "text-gray-500"}`}
                    >
                      {item.description}
                    </p>
                  )}
                  ...
                  {isGame && item.badges && (
                    <div className="flex flex-wrap gap-3 ml-8 mt-2">
                      {item.badges.map((badge, bIndex) => (
                        <span
                          key={bIndex}
                          className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs md:text-sm font-bold uppercase tracking-widest ${badge.style}`}
                        >
